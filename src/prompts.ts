...
export function buildRoleAnnouncement(role: Role): string {
  return [
    ...
    "---",
    LANGUAGE_INSTRUCTION,
    "Start your next reply with this line, with nothing before it, keeping the",
    `"${PERSONA_PREFIX}" prefix verbatim and translating the rest into that reply language:`,
    `${PERSONA_PREFIX} active role — ${ROLE_LABEL[role]}.`,
    "Then handle the user's request, applying the role instructions above",
    "for the whole session. Do not repeat this announcement.",
